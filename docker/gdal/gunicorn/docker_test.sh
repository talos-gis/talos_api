...
sudo docker run \
  ...
  --name talos_wps_test \
  --env WORKDIR=/app \
  --mount type=bind,source=/home/idan/maps,target=/app/static/data/maps,readonly \
  --mount type=bind,source=/home/idan/dev/gis/talos_wps/data/config,target=/app/data/config,readonly \
  talos_wps:latest
